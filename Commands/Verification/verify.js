        ...
        module.exports = {
          ...
          async execute(message, args) {
            // IDs of the roles to give upon successful verification
            const verifiedRoleIds = ['1223600860643196950', '1220958223998124052'];

            // Check if the user already has any of the verified roles
            const hasVerifiedRole = verifiedRoleIds.some(roleId => message.member.roles.cache.has(roleId));
            if (hasVerifiedRole) {
            ...
            const sendVerificationDM = async () => {
              try {
                ...
                const verificationEmbed = new EmbedBuilder()
                  ...
                  .setColor(0x8B0000); // Example color, adjust as necessary
                ...
                collector.on('collect', m => {
                  if (m.content.toUpperCase() === randomImage.answer) {
                    // Correct answer, add the verified roles
                    verifiedRoleIds.forEach(roleId => {
